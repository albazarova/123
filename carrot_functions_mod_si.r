#bug for multin
#stationary variable in regr_ind
...
cross_val<-function(vari,outi,c,rule,part,l,we,vari_col,preds,preds_si,mode,cmode,predm,cutoff,objfun,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL){
  ...
  for (q in 1:vari_col){
    ...
    if (mode=='linear'){
      ...
      re=summary(multinom(outc~.,data=data.frame(vari[ra,q]),trace=FALSE))
    ...
    if (min(re[2:length(re)])<=0.05) signi=c(signi,q);
  ...
  if ((mode=='binary')&(objfun=='acc')){
    ...
    if ((mode=='multin')|(objfun=='roc')) {
      ...
      if (mode=='multin')  predsi=1-length(which(abs(outi[setdiff(1:l,ra)]-array(predsi))>0))/(l-floor((1-1/part)*l));
      ...
      predsi=sum(abs(outi[setdiff(1:l,ra)]-predsi)/(l-floor((1-1/part)*l)));
      
      
      predsir=(sum(abs((outi[setdiff(1:l,ra)]-predsi)/outi[setdiff(1:l,ra)]))/(l-floor((1-1/part)*l)));
      
      list("regr" = preds, "nvar"=nvar, "regrr"=predsr, "emp"=cpred,"empr"=cpredr,"regrsi"=predsi, "regrsir"=predsir,"regr_si"=preds_si,"regr_sir"=preds_sir)
...
regr_ind<-function(vari,outi,crv,cutoff=NULL,part=10,mode,cmode='det',predm='exact',objfun='acc',parallel=FALSE,cores,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL){
  ...
  if ((mode=='binary')&(objfun=='acc')) {
    ...
    if ((mode=='multin')|(objfun=='roc')) {
      ...
      print(c(max(predsp[predsp>0],na.rm=TRUE),sum(predsi[predsi<Inf])/(crv-length(which(predsi==Inf)))))
