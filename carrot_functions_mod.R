...
make_numeric_sets<-function(a,ai,k,vari,ra,l){
  ...
  for (m in 1:length(a[,ai[k]])){
    ...
    if (is.numeric(vari[,a[m,ai[k]]])==FALSE){
      ...
      anum1<-data.frame(make_numeric(vari[,a[m,ai[k]]],FALSE,ra))
      anum<-data.frame(make_numeric(vari[,a[m,ai[k]]],FALSE,setdiff(1:l,ra)))
      ...
      testset1<-data.frame(cbind(testset1,vari[setdiff(1:l,ra),a[m,ai[k]]]))
      trset1<-data.frame(cbind(trset1,vari[ra,a[m,ai[k]]]))
