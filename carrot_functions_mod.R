...
#Improvements: Added minx and maxx to regr_ind,cross_val (do the same to regr_whole!). Categorical non-numerical should not be treated as numerical in case of more than 5 categories!
#Added non-random part of the randomisation to cross_val
...
cross_val<-function(vari,outi,c,rule,part,l,we,vari_col,preds,mode,cmode,predm,cutoff,objfun,minx=NULL,maxx=NULL,nr=NULL){
...
regr_ind<-function(vari,outi,crv,cutoff=NULL,part=10,mode,cmode='det',predm='exact',objfun='acc',parallel=FALSE,cores,minx=NULL,maxx=NULL){
  ...
  result <- foreach(i=1:crv, .combine='comb', .multicombine=TRUE) %fun% {
    ...
    cross_val(vari,outi,c,rule,part,l,we,vari_col,preds,mode,cmode,predm,cutoff,objfun,minx,maxx);
