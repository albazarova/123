...
#'
#'Function which turns a categorical (non-numeric) variable into a numeric one (or several) by introducing dummy '0'/'1' variables.
#'
...
#'@return Returned value is a matrix MxN where M is the length of the input array of indices \code{ra} and N is \code{length(vari)-1}.
...
#'
#'@param a matrix MxN, containing all possible subsets (N overall) of the size M of predictors' indices; therefore each column of \code{a} defines a unique subset of the predictors
...
#'@return returns anobject containing two objects: \code{tr} and \code{test}\cr
#'\item{tr}{training set transformed into numeric} 
#'\item{test}{test set transformed into numeric}
...
#'
#'Function which computes the weights of each predictor according to the rules of thumb and outputs it into corresponding array
#'
#'@param vari_col number of predictors
#'@param vari set of predictors
#'@details Continuous or multinomial variable with more then 5 categories has a weight 1, otherwise it has a weight \code{n-1} where \code{n} is the number of categories 
...
#' 
#'Function which computes maximal weight of the predictors to be included into a regression according to the rule of thumb applied to the outcome variable
#'
#'@details For continuous outcome variable it equals sample size divided by 10, for multinomial it equals the size of the smallest category divided by 10
...
#'@return returns an integer value of maximum aloud weight
#'@export compute_max_weight
#'@examples
#' compute_max_weight(1:23,'binary')
#'@references{
#'\insertRef{ref1}{CARRoT}
#'}
#'@importFrom Rdpack reprompt
...
#'
#'Function which finds predictors' subsets determined by an array \code{s}
#'@param a matrix MxN, containing all possible subsets (N overall) of the size M of predictors' indices; therefore each column of \code{a} defines a unique subset of the predictors
#'@param s indices of the columns of \code{a} to be returned
#'@param j size of the predictors' subset
...
#'
#'Function which computes the maximum number of regressions with the same number of variables in them based on the rule of thumb
#'
#'@param vari_col number of predictors
...
#'@param we array of weights of the predictors
#'@import utils
#'@return Integer number correponding to maximum number of regressions of the same size
...
#'
#'Function which computes probabilities of outcomes of the test set by applying regression parameters inferred by a run on the training set. Works for logistic or multinomial regression
#'@param trset predictors of the training set
#'@param testset predictors of the test set
#'@param outc outcomes of the training set
...
#'@return Probabilities of the outcomes. In \code{'binary'} mode returns an array of the size of the number of observations in a testset. In \code{'multin'} returns matrix MxN where M is the size of the number of observations in a testset
...
#'@param testset predictors of the test set
#'@param outc outcomes of the training set
#'@param k length of the training set
#'@usage get_predictions_lin(trset,testset,outc,k)
#'@return An array of continous variables of the size of a \code{testset}
...
#'@param p probabilities of the outcomes for the test set given wither by an array (logistic regression) or by a matrix (multinomial regression)
#'@param k number of observations in the test set
#'@param cutoff cut-off value of the probability
#'@param cmode \code{'det'} or \code{''}; \code{'det'} computes prediction based on maximal probability of the event; \code{''} samples randomly from the binomial/multinomial (for logistic/multinomial) distribution with given probbaility to obtain a prediction
...
#'
#'Function computing area under the curve fast taken from \url{https://stat.ethz.ch/pipermail/r-help/2005-September/079872.html}
...
#'
#'Function running one cross-validation by partitioning the data into training and test set
#'
#'@param vari set of predictors
#'@param outi array of outcomes
#'@param c set of all indices of the predictors
#'@param rule rule of 10 in this case
#'@param part partitions the set into training and test set in a proportion \code{(part-1):1}
...
#'@param cmode \code{'det'} or \code{''}; \code{'det'} computes prediction based on maximal probability of the event; \code{''} samples randomly from the binomial/multinomial (for logistic/multinomial) distribution with given probbaility to obtain a prediction
#'@param mode \code{'binary'} (logistic regression), \code{'multin'} (multinomial regression) 
#'@param predm \code{'exact'} or \code{''}; for logistic and multinomial regression; \code{'exact'} computes home many times exact category was predicted, \code{''} computes how many times a category was predicted correctly up to a class
...
#'@return 
#'\item{regr}{matrix MxN of sums of the absolute errors for each element of the test set for each feasible regression. M is the maximum feasible number of regressions of the fixed size, N is maximum feasible number of variables for the generated partition into a test and training set. Therefore rows correspond to results obtained from running regressions with the same number of variables and columns - to different subsets of predictors used.} 
#'\item{regrr}{matrix MxN of sums of the relative errors for each element of the test set (only for \code{mode = 'linear'}) for each feasible regression. M is the maximum feasible number of regressions of the fixed size, N is maximum feasible number of variables for the generated partition into a test and training set. Therefore rows correspond to results obtained from running regressions with the same number of variables and columns - to different subsets of predictors used.}
#'\item{nvar}{maximum feasible number of variables in the regression}
#'\item{emp}{an accuracy of the empirical prediction (only for \code{mode = 'binary'} and \code{objfun = 'acc'})}
...
#'@param preds list of the size of all cross-validations of MxN matrices of predictive powers for all feasible regressions (M is the maximum feasible number of regressions of the fixed size, N is maximum feasible number of variables for the generated partition into a test and training set)
#'@param crv number of cross-validations
#'@param k size of the test set
#'@usage av_out(preds,crv,k)
#'@return Returns a MxN matrix of averaged out predictive powers where M is the maximum feasible number of regressions of the fixed size, N is maximum feasible number of variables for the generated partition into a test and training set
#'@export av_out
...
#'Best regression
#'
#'Function which identifies regressions with the best predictive power
#'
#'@param predsp MxN matrix of averaged out predictive power values. M is the maximum feasible number of regressions of the fixed size, N is maximum feasible number of variables for the generated partition into a test and training set.
...
#' 
#'One of the two main functions of the package. Identifies the predictors included into regressions with the best predictive power
#'@param vari set of predictors
#'@param outi array of outcomes
#'@param crv number of cross-validations
...
#'@return Prints the best predictive power provided by a regression, predictive accuracy of the empirical prediction (for logistic regression). Returns indices of the predictors included into regressions with the best predictive power written in a list. For \code{mode='linear'} outputs a list of two lists. First list corresponds to the smallest absolute error, second corresponds to the smallest relative error 
#'@export regr_ind
#'@import doParallel
#'@import foreach
#'@import parallel
...
#'regr_ind(variables_lin,outcomes_lin,1000,mode='linear',parallel=TRUE,cores=2)
...
#' Best regressions
#' 
#' Function which prints the best predictive power, predictive accuracy of the empirical prediction (for logistic regression), outputs the regression objects corresponding to the best predictive power and the indices of the variables included into regressions with the best predictive power
#' For linear regression it outputs the best regressions with respect to both absolute and relative errors
#'@param vari set of predictors
#'@param outi array of outcomes
#'@param crv number of cross-validations
#'@param cutoff cut-off value for mode \code{'binary'}
...
#'@param parallel TRUE if using parallel toolbox, FALSE if not. Defaults to FALSE
#'@param cores number of cores to use in case of parallel=TRUE
#'@return Prints the best predictive power provided by a regression, predictive accuracy of the empirical prediction (for logistic regression). 
...
#'regr_whole(vari,out,1000,cutoff=0.5,part=10,mode='binary',parallel=TRUE,cores=2)
